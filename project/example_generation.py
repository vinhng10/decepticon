...
if __name__ == "__main__":
    ...
    fx_model = RaceModule.load_from_checkpoint("D:/Github/decepticon/project/models/ckpts/t5d.ckpt")
    fx_model.setup_tune(top_p = 0.95, top_k = 50, no_repeat_ngram_size = 2, num_samples = 10)

    fx_model.eval()
    f = open('sample_generation.txt','w')
    for x,y in fx_dm.test_dataloader():
        output = fx_model.generate(x)
        display_result_as_string(fx_dm.tokenizer, None, output, x["input_ids"])
